...
An Auto Scaling Group for ECS worker nodes (in private subnets)

An Application Load Balancer (in public subnets)

An ECS Service running the Docker image

Supporting networking and IAM roles

ECS tasks run in private subnets, traffic comes in via the ALB in the public subnets.
...
Open that URL in your browser to see the JSON response from SimpleTimeService.
...
Security groups and IAM roles created by the config
...
Backend configured in provider.tf:
...
Remote, centralized Terraform state
...
A Jenkinsfile is included at the root of the repo.

The pipeline is designed to:
Check out the repository
-Build the Docker image from app/
-Push the image to DockerHub
-run Terraform commands like init, fmt, and plan from terraform/
-Jenkins is configured to use a DockerHub credential (e.g. dockerhub-creds) for logging in and pushing the image. No credentials are stored in this repository.
...
-ECS tasks are placed in private subnets for better security
-ALB exposes the service Externally
-No secrets or keys are committed to this repository
-Terraform can deploy and destroy the full stack using terraform apply and terraform destroy
